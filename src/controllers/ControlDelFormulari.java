 ...
 */
@WebServlet("/ControlDelFormulari")
public class ControlDelFormulari extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public ControlDelFormulari() {
        super();
    }
	...
	protected void doGet(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
		   ...
		   if (user.isComplete()) {
			   ...
			   try {
				   DAO dao = new DAO();
				   String query2="INSERT INTO users VALUES ('"	+user.getUser()+"', '"
						   										+user.getName()+"', '"
						   										+user.getMiddleName()+"','"
						   										+user.getLastName()+"','"
						   										+user.getMail()+"','"
